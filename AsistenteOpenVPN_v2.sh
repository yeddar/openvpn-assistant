...
if [[ "$EUID" -ne 0 ]]; then
	echo "Error. Tienes que ser administrador para ejecutar este asistente."
	exit
fi
...
key-direction 1" > ~/client-configs/base.conf
		...
		"$iniciar")
		systemctl start openvpn@server
		srv=$(ps awx | grep 'openvpn' | grep -v grep | wc -l)
		if [[ $srv == 0 ]]; then
		    echo "Ha ocurrido un error al arrancar el servicio."
		    systemctl status openvpn@server
		    
		else
		    echo "Servicio activo."
		fi
		read -n1 -r -p "Presiona una tecla para volver al menú..."
		;;
		"$detener")
		systemctl stop openvpn@server
		srv=$(ps awx | grep 'openvpn' | grep -v grep | wc -l)
